...
function AdminSharedGoalsWorkspace({ adminId }: { adminId: string }) {
  const [sharedGoals, setSharedGoals] = useState<SharedGoalWithAssignments[]>(() =>
    getSharedGoalsForAdmin()
  );
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const employees = mockUsers.filter((user) => user.role === ROLES.EMPLOYEE);
  ...
  function handleCreateSharedGoal(input: SharedGoalCreateInput) {
    const result = createSharedGoal(input, adminId);
  ...
  function handleActivateSharedGoal(goalId: string) {
    ...
    const result = activateSharedGoal(goal, adminId);
  ...
  function handleChangeWeightage(
    ...
    const result = updateSharedGoalWeightage(goal, employeeId, weightage, adminId);
    ...
    <div className="w-full space-y-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                Govern departmental KPIs, linked assignments, and primary owner
                achievement sync across the organization.
              </p>
            </div>
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {mockDepartments.length} departments available
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SharedGoalMetricCard
          helper="Departmental KPIs governed in the organization workspace."
          icon={Share2}
          label="Shared goals"
          tone="blue"
        ...
        />
        <SharedGoalMetricCard
          helper="Employees currently linked to shared goal sheets."
          icon={UsersRound}
        ...
        />
        <SharedGoalMetricCard
          helper="Draft goals still waiting to be pushed."
          icon={AlertCircle}
          label="Draft goals"
          ...
          <CardContent className="space-y-5 p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h2 className="text-xl font-semibold tracking-normal text-slate-950">
                  Organization shared goals
                </h2>
                <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">
                  Active shared goals appear on linked employee goal sheets with
                  read-only title, target, measurement, and thrust area.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Badge className="gap-1.5 border-transparent bg-emerald-50 text-emerald-700">
                  ...
                  {syncedAssignments}/{assignmentCount} synced
                </Badge>
              </div>
            </div>
            ...
            {sharedGoals.length > 0 ? (
              ...
              <SharedGoalEmptyState
                ...
                description="Create a departmental KPI and assign it to linked employees."
              />
            )}
          </CardContent>
        </Card>
...
function AdminSharedGoalsContent() {
  ...
  return (
    <AdminSharedGoalsWorkspace
      key={sessionState.session.user.id}
      adminId={sessionState.session.user.id}
    />
  );
}
